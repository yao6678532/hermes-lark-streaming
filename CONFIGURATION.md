...
| 工具详情模式 | `display.platforms.feishu.tool_detail_mode` | `full` / `compact` | `full` | 运行时重新读取 | `full` 保留当前完整 sanitized detail；`compact` 对 Terminal/command 优先显示 executable/script 和安全 subcommand，隐藏长参数 payload；其他工具保守使用 sanitized detail。非法值 fallback 到 `full`。也兼容 `display.tool_detail_mode` 作为 fallback。 |
