...
def _background_review_hook(indent: str) -> str:
    return _make_hook(
        ...
        [
            ...
            "    _lark_bg_review_platform = getattr(getattr(ctx, 'source', None), 'platform', None)",
            "    _lark_bg_review_platform_name = getattr(_lark_bg_review_platform, 'value', _lark_bg_review_platform)",
            "    _lark_bg_review_is_feishu = str(_lark_bg_review_platform_name or '').lower() in ('feishu', 'lark')",
            "    _lark_bg_review_chat_id = str(ctx._status_chat_id or '')",
            "    _lark_bg_review_conversation_key = 'feishu:' + _lark_bg_review_chat_id",
            "    _lark_bg_review_profile_home = (",
            "        self._runner._resolve_profile_home_for_source(ctx.source)",
            "        if _lark_bg_review_is_feishu else None",
            "    )",
            "    def _lark_bg_review_callback(message):",
            "        if not _lark_bg_review_is_feishu:",
            "            _lark_bg_review_sender(message)",
            "            return",
            "        _lark_bg_review_owned = on_background_review_message(",
            "            conversation_key=_lark_bg_review_conversation_key,",
            "            chat_id=_lark_bg_review_chat_id,",
            "            profile_home=_lark_bg_review_profile_home,",
            "            text=message,",
            "        )",
            "        if not _lark_bg_review_owned:",
            "            _lark_bg_review_sender(message)",
